...
# Dashboard layout
st.title("📈 Enhanced Funnel Dashboard with US/Non-US Filter")
st.markdown("Visualize drop-offs across Marketing and Sales pipelines with filters.")

col1, col2 = st.columns(2)

with col1:
    st.subheader("Marketing Funnel: MQL → Opportunity")
    fig1 = px.funnel(mql_funnel, x="Count", y="Stage", text="Count")
    st.plotly_chart(fig1, use_container_width=True)

with col2:
    st.subheader("Sales Funnel: Opportunity → Closed Won")
    fig2 = px.funnel(sales_funnel, x="Count", y="Stage", text="Count")
